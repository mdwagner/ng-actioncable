  ...
  constructor(public url: string, public params?: any) {
    ...
    if (params instanceof Function) {
      this.disconnected().subscribe(() => {
        this.baseCable.url = (ActionCable as any).createWebSocketURL(this.buildUrl(url, params));
      });
    }
  }
...
export class Channel {
  ...
  constructor(public cable: Cable, public name: string, public params = {}) {
    ...
    this.messages = new Observable((observer) => {
      ...
      this.eventTypes.forEach((type) => {
        mixin[type] = (data: any) => (this as any)[`${type}Source`].next(data);
      });
  ...
  unsubscribe(): void {
    ...
    this.eventTypes.forEach(type => (this as any)[`${type}Source`].complete());
  }
}
